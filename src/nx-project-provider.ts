...
export class NxProvider {
  static async getNxProjects(loadInParallel = true): Promise<NxProjInfo[]> {
    ...
    try {
      ...
      const projectNames = await execAsyncParse<string[]>(
        `${nxCmd} show projects --json`
      );
  ...
  private static async loadProjectsSeq(projectNames: string[], nxCmd: string) {
    ...
    for (const projectName of projectNames) {
      const details = await execAsyncParse<NxProjInfo>(
        `${nxCmd} show project ${projectName} --json`
      );
  ...
  private static loadProjectsParallel(
    projectNames: string[],
    nxCmd: string
  ): Promise<NxProjInfo[]> {
    const promises = projectNames.map((projectName) =>
      execAsyncParse<NxProjInfo>(`${nxCmd} show project ${projectName} --json`)
    );
...
async function findNx(): Promise<string | null> {
  ...
  vscode.window.showErrorMessage(
    "Nx is not installed. Have you run npm/yarn install?"
  );
